-- ViandApp · 0020 · Fixes familia
-- 1) Cancel: padre puede cancelar pedidos en estado pendiente_pago también
--    (el pago MP nunca se confirmó, libera el día sin tocar plata)
-- 2) Eliminar hijo: el padre puede desvincularse de un alumno

-- ========================================
-- 1. fn_borrar_pedido: aceptar pendiente_pago para padre
-- ========================================
CREATE OR REPLACE FUNCTION public.fn_borrar_pedido(p_pedido_id uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $$
DECLARE
  v_usuario_id uuid := auth.uid();
  v_rol text;
  v_alumno_id uuid;
  v_fecha date;
  v_estado text;
  v_ahora timestamptz := now();
  v_hoy_ar date := (v_ahora AT TIME ZONE 'America/Argentina/Buenos_Aires')::date;
  v_hora_ar time := (v_ahora AT TIME ZONE 'America/Argentina/Buenos_Aires')::time;
  v_mov record;
BEGIN
  SELECT rol INTO v_rol FROM public.usuarios WHERE id = v_usuario_id AND activo = true;

  SELECT alumno_id, fecha, estado
    INTO v_alumno_id, v_fecha, v_estado
    FROM public.pedidos WHERE id = p_pedido_id;
  IF v_alumno_id IS NULL THEN
    RAISE EXCEPTION 'Pedido no encontrado: %', p_pedido_id;
  END IF;

  IF v_rol = 'padre' THEN
    IF NOT public.es_alumno_propio(v_alumno_id) THEN
      RAISE EXCEPTION 'Este pedido no es de un alumno tuyo';
    END IF;
    IF v_fecha < v_hoy_ar THEN
      RAISE EXCEPTION 'No se pueden cancelar pedidos de días pasados';
    END IF;
    IF v_fecha = v_hoy_ar AND v_hora_ar >= time '08:00' THEN
      RAISE EXCEPTION 'No podés cancelar el pedido después de las 8:00 del día de la vianda';
    END IF;
    -- Permitimos cancelar tanto confirmado como pendiente_pago.
    -- Si está pendiente_pago: simplemente libera el día (el pago MP nunca se
    -- confirmó, así que no hay plata para devolver).
    IF v_estado NOT IN ('confirmado','pendiente_pago') THEN
      RAISE EXCEPTION 'El pedido ya está en estado %', v_estado;
    END IF;
  ELSIF v_rol NOT IN ('operadora','encargada','administrativo') THEN
    RAISE EXCEPTION 'No autorizado';
  END IF;

  -- Reverso de movimientos (igual que antes)
  FOR v_mov IN
    SELECT id, tipo, monto, bucket_id, pedido_producto_id
      FROM public.movimientos_credito
     WHERE pedido_id = p_pedido_id
     ORDER BY created_at DESC
  LOOP
    IF v_mov.tipo IN ('consumo_vianda','consumo_producto') THEN
      IF v_mov.bucket_id IS NOT NULL THEN
        UPDATE public.creditos_buckets
           SET pesos_restantes = pesos_restantes + (-v_mov.monto),
               agotado = false
         WHERE id = v_mov.bucket_id;
      END IF;
      INSERT INTO public.movimientos_credito (
        alumno_id, tipo, monto, pedido_id, pedido_producto_id, bucket_id,
        usuario_id, nota
      ) VALUES (
        v_alumno_id,
        CASE v_mov.tipo
          WHEN 'consumo_vianda'   THEN 'reverso_consumo_vianda'
          WHEN 'consumo_producto' THEN 'reverso_consumo_producto'
        END,
        -v_mov.monto,
        p_pedido_id, v_mov.pedido_producto_id, v_mov.bucket_id,
        v_usuario_id, 'Reverso por cancelación'
      );

    ELSIF v_mov.tipo = 'deuda_vianda' THEN
      INSERT INTO public.movimientos_credito (
        alumno_id, tipo, monto, pedido_id, usuario_id, nota
      ) VALUES (
        v_alumno_id, 'reverso_deuda_vianda', -v_mov.monto,
        p_pedido_id, v_usuario_id, 'Reverso por cancelación'
      );

    ELSIF v_mov.tipo = 'deuda_producto' THEN
      INSERT INTO public.movimientos_credito (
        alumno_id, tipo, monto, pedido_id, pedido_producto_id, usuario_id, nota
      ) VALUES (
        v_alumno_id, 'reverso_deuda_producto', -v_mov.monto,
        p_pedido_id, v_mov.pedido_producto_id, v_usuario_id, 'Reverso por cancelación'
      );
    END IF;
  END LOOP;

  DELETE FROM public.pedidos WHERE id = p_pedido_id;
  PERFORM public._recalcular_alumno(v_alumno_id);
END;
$$;

-- ========================================
-- 2. fn_desvincular_alumno: el padre se desvincula de un alumno
-- Cancela TODOS los pedidos futuros del padre→alumno y borra la fila en
-- familias_alumnos. Si era el único padre vinculado, marca el alumno como
-- inactivo. No borra nada de history (pedidos pasados quedan para auditoría).
-- ========================================
CREATE OR REPLACE FUNCTION public.fn_desvincular_alumno(p_alumno_id uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $$
DECLARE
  v_uid uuid := auth.uid();
  v_rol text;
  v_vinculo_existe boolean;
  v_otros_padres integer;
  v_hoy date := (now() AT TIME ZONE 'America/Argentina/Buenos_Aires')::date;
  v_pedido record;
BEGIN
  SELECT rol INTO v_rol FROM public.usuarios WHERE id = v_uid AND activo = true;
  IF v_rol <> 'padre' THEN
    RAISE EXCEPTION 'Solo un padre puede desvincularse de un alumno';
  END IF;

  SELECT EXISTS(
    SELECT 1 FROM public.familias_alumnos
     WHERE usuario_id = v_uid AND alumno_id = p_alumno_id
  ) INTO v_vinculo_existe;

  IF NOT v_vinculo_existe THEN
    RAISE EXCEPTION 'No estás vinculado a este alumno';
  END IF;

  -- Cancelar pedidos futuros del alumno (cualquier padre) para que no quede
  -- una vianda "huérfana" si nadie más vigila. Se devuelve saldo / libera día.
  FOR v_pedido IN
    SELECT id FROM public.pedidos
     WHERE alumno_id = p_alumno_id
       AND fecha >= v_hoy
       AND estado IN ('confirmado','pendiente_pago')
  LOOP
    -- Reverso simplificado: igual que fn_borrar_pedido pero sin checks
    DECLARE
      v_mov record;
    BEGIN
      FOR v_mov IN
        SELECT id, tipo, monto, bucket_id, pedido_producto_id
          FROM public.movimientos_credito
         WHERE pedido_id = v_pedido.id
         ORDER BY created_at DESC
      LOOP
        IF v_mov.tipo IN ('consumo_vianda','consumo_producto') THEN
          IF v_mov.bucket_id IS NOT NULL THEN
            UPDATE public.creditos_buckets
               SET pesos_restantes = pesos_restantes + (-v_mov.monto),
                   agotado = false
             WHERE id = v_mov.bucket_id;
          END IF;
        END IF;
      END LOOP;
      DELETE FROM public.pedidos WHERE id = v_pedido.id;
    END;
  END LOOP;

  -- Desvincular este padre
  DELETE FROM public.familias_alumnos
   WHERE usuario_id = v_uid AND alumno_id = p_alumno_id;

  -- ¿Quedan otros padres vinculados?
  SELECT count(*) INTO v_otros_padres
    FROM public.familias_alumnos
   WHERE alumno_id = p_alumno_id;

  -- Si era el único, marcar al alumno como inactivo
  IF v_otros_padres = 0 THEN
    UPDATE public.alumnos SET activo = false, updated_at = now()
     WHERE id = p_alumno_id;
  END IF;

  PERFORM public._recalcular_alumno(p_alumno_id);
END;
$$;

GRANT EXECUTE ON FUNCTION public.fn_desvincular_alumno(uuid) TO authenticated;
